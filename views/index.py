...
from plugins.common import allowCROS, noCache
from plugins.common import settings, token_auth
from views.models import mutex, zhishiku, LLM
import logging
...
def index():
    if settings.key:
        return '已开启外部认证, 需使用token登入'
    noCache()
    return static_file("index.html", root="views/templates/")


@route('/<visitor>/<stamp>/<token>')
def index2(visitor, stamp, token):
    @token_auth
    def _(*args):
        noCache()
        return static_file("index.html", root="views/templates/")

    return _(visitor, stamp, token)
